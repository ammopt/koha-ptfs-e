...
open my $handle, '<', $csv_file or die "Cannot open $csv_file :$!";
...
"ERROR: missing critical column data '$key' for line $.: $borrowerline\n";
    ...
    if ( $borrower{userid} && $borrower{userid} eq q{} ) {
...
# Popular spreadsheet applications make it difficult to force date outputs to be zero-padded, but we require it.
    foreach (qw(dateofbirth dateenrolled dateexpiry)) {
        ...
        $tempdate = eval { output_pref(
                { dt => dt_from_string( $tempdate ), dateonly => 1, dateformat => 'iso', }
            ); };
        if ( $tempdate ) {
            $borrower{$_} = $tempdate;
        }
    ...
    if (
        C4::Members::checkcardnumber( $borrower{cardnumber}, $borrowernumber ) )
    {
        $borrowernumber ||= q{};
        print
"ERROR: invalid cardnumber '$borrower{cardnumber}' for borrowernumber $borrowernumber\n";
        $invalid++;
        next;
    }
    ...
    if ($borrowernumber) {
        ...
        unless ($overwrite_cardnumber) {
            $alreadyindb++;
            print "$borrower{'surname'} / $borrowernumber already in database.\n"
              if $verbose;
            next LINE;
        }
...
# untill we have better error trapping, we have no way of knowing why ModMember errored out...
        ...
        if ($extended) {
            if ($ext_preserve) {
                ...
                $patron_attributes = extended_attributes_merge( $old_attributes,
                    $patron_attributes );
            }
        ...
        print
          "Overwriting $borrower{'surname'} / $borrowernumber with new data\n";
        $overwritten++;
    }
    else {
