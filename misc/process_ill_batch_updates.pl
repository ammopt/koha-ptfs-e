...
my $batches = Koha::Illbatches->search($where);

debug_msg("Processing " . $batches->count . " batches");
...
foreach my $processor(@{$backend->{batch_processors}}) {
    debug_msg("- Processor " . $processor->{name});
    my $update_log = {
        processed_by   => $backend->name,
        processors_run => []
    };
    $processor->run(undef, $options_to_pass);
