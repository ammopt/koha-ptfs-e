...
if( CheckVersion( $DBversion ) ) {
    $dbh->do(q{
        CREATE TABLE IF NOT EXISTS l10n_source (
            ...
            UNIQUE KEY group_text (`group`, `text`(60))
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
    });

    $dbh->do(q{
        CREATE TABLE IF NOT EXISTS l10n_key (
            `l10n_key_id` INT(11) NOT NULL AUTO_INCREMENT,
            `l10n_source_id` INT(11) NOT NULL,
            `group` VARCHAR(100) NULL DEFAULT NULL,
            `key` VARCHAR(100) NOT NULL,
            PRIMARY KEY (l10n_key_id),
            UNIQUE KEY group_key (`group`, `key`),
            CONSTRAINT `l10n_key_fk` FOREIGN KEY (`l10n_source_id`) REFERENCES `l10n_source` (`l10n_source_id`)
    ...
    $dbh->do(q{
        INSERT IGNORE INTO l10n_source (`group`, `text`)
        SELECT DISTINCT 'itemtype', description FROM itemtypes
    });

    $dbh->do(q{
        INSERT IGNORE INTO l10n_key (`l10n_source_id`, `key`)
        SELECT `l10n_source`.`l10n_source_id`, `itemtypes`.`itemtype` FROM `itemtypes` `itemtypes` JOIN `l10n_source` `l10n_source` ON ( `itemtypes`.`description` = `l10n_source`.`text`);
    });
